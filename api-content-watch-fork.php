...
Author: hmjim
Version: 1.0.1
Description: Форк от официального плагина с дабвлением таблицы постов в админке
...
class ContentWatchPlugin {
	...
	public function init() {
		...
		add_filter( 'option_page_capability_' . 'checktext', 'page_capability' );

		// Добавим видимость пункта меню для Редакторов
		function register_my_page() {
			add_menu_page( 'Проверка текста', 'Проверка текста', 'edit_others_posts', 'checktext', 'page_capability', 'dashicons-admin-tools', 6 );
		}
		...
		function page_capability( $capability ) {
			...
			class My_List_Table extends WP_List_Table {
				...
				function prepare_items() {
					...
					$args         = array(
						...
						'post_status'    => 'publish',
						'order'          => 'DESC',
						'posts_per_page' => 30 // this will retrive all the post that is published
					);
					...
					if ( $result->have_posts() ) {
						while ( $result->have_posts() ) {
							...
							$tmp_data = array(
								...
								'check'     => get_post_meta( get_the_id(), 'content-watch', 1 ),
							);
							// $this->items = $tmp_data;
							// array_push($example_data, $tmp_data);
							array_push( $example_data, $tmp_data );
						}
					}
					wp_reset_postdata();
			...
			echo '<div class="wrap"><h2>Проверка текста</h2>';
...
HTML;
	...
	public function addMetaBox() {
		...
		foreach ( $screens as $screen ) {
			add_meta_box( 'cw_plagiat_box', 'Content-watch', array(
				$this,
	...
	protected function getPostMatchesHTML( $postId ) {
		...
		// $to = 'info@bit.news';
		// $zag = 'Проверка новости на уникальность c id: ' . $postId;
		// $subject = $zag;
		// $message = $this->formatMatchesHTML($matches);
	...
	public function checkPost( $postId, $text = null ) {
		...
		$current_text_file = '</br></br>' . 'Time: ' . date( "Y-m-d H:i:s" ) . '</br>';
		$current_text_file .= $text_done . '</br>' . PHP_EOL;
		...
		$cachefile = ABSPATH . '/checktext/' . $postId . '.txt';
